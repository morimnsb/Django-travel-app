...
const UserPanel = () => {
  ...
  useEffect(() => {
    const fetchUserDetails = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`http://localhost:8000/user/search/${username}`);
        console.log("Backend Response:", response);
    
        const responseData = response.data;
        if (!responseData.success || !responseData.data || !responseData.data.userInfo) {
        ...
        let parsedPosts;
        try {
          parsedPosts = JSON.parse(responseData.data.userInfo.posts || '[]');
        } catch (parseError) {
          console.error("Error parsing posts field:", parseError);
          return;
        }
        ...
        setUser({
          ...
          avatar: responseData.data.userInfo.avatar || "/assets/avatar.png",
          posts: formattedPosts,
        });
    
        updateAvatar(responseData.data.userInfo.avatar);
      } catch (error) {
            ...
            {user.posts && user.posts.length > 0 ? (
              ...
              >
                {user.posts.map((post) => (
                  ...
                  >
                    {/* Adjust the rendering of post details based on your data structure */}
                    <div className="p-5">
                      <h5 className="text-2xl dark:text-white">{post.title}</h5>
                      <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
                        {post.description.slice(0, 100)}...
                      </p>
                      <Link to={`/posts/details/${post.id}`} className="my-1">
